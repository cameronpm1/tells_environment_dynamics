...
class droneDynamics(baseDynamics):
    ...
    def __init__(
        ...
        '''
        EOM for quadcopter using quaternions for orientation:
        https://ieeexplore.ieee.org/abstract/document/9674884
        ...
        if inertial_data is not None:
            self.I11 = inertial_data['J_qc'][0]
            self.I22 = inertial_data['J_qc'][1]
            self.I33 = inertial_data['J_qc'][2]
            self.mass = inertial_data['mass']
            self.l = inertial_data['arm_length']
            self.kf = inertial_data['k_f']
            self.km = inertial_data['k_m']
            self.Im = inertial_data['J_r']
        else:
    ...
    def B(self): 
        ...
        R = self.get_dcm()
        B[3:6,0:3] = R/self.mass
        B[6,3] = 1/self.I11
        B[7,4] = 1/self.I22
        B[8,5] = 1/self.I33
        omega1 = self.state[6]
        omega2 = self.state[7]
        B[6,6] = -self.Im*omega2/self.I11
        B[7,6] = self.Im*omega1/self.I22
        return B
    ...
    def set_control(
        ...
        control:list[float]
            control = [w1, w2, w3, w4]
        '''
        self.control[2] = self.kf * (control[0]**2 + control[1]**2 + control[2]**2 + control[3]**2) #Thrust        
        self.control[3] = self.l * self.kf * (control[1]**2 - control[3]**2) #Mx
        self.control[4] = self.l * self.kf * (control[2]**2 - control[0]**2) #My
        self.control[5] = self.km * (control[0]**2 - control[1]**2 + control[2]**2 - control[3]**2) #Mz
        self.control[6] = control[0] - control[1] + control[2] - control[3] #motor inertia term
    ...
    def compute_derivatives(
        ...
        dxdt[0:3] = vel
        dxdt[3:6] = np.matmul(state_matrix[3:6, :], self.state) + np.squeeze(np.matmul(control_matrix[3:6, :], self.control)) + np.array([0.0,0.0,-self.g])
